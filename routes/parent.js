...
router.get('/failed', check_auth, async (req, res, next) => {
    try {
        ...
        res.json({
            success: true,
            message: 'List of fAILED Parents',
            users: users,
        })
    } catch (error) {
        next(error)
...
router.get('/basedOnMonth', check_auth, async (req, res, next) => {
    ...
    try {
        ...
        res.json({
            success: true,
            message: 'Number of Parents based on their registered Month',
            numberinMonths: years,
        })
    } catch (error) {
        next(error)
    }
